...
if(isset($_POST["submit"])){
  ...
  $duplicate = mysqli_query($conn, "SELECT * FROM tbl_users WHERE username = '$EnterYourUsername' OR email = '$EnterYourEmail'");
  if(mysqli_num_rows($duplicate) > 0){
    echo "<script> alert('Username or Email Has Already Taken'); </script>";
  }
  else{
    if($CreateaPassword == $Confirmpassword){
      $query = "INSERT INTO tbl_users VALUES('','$EnterYourUsername','$EnterYourName','$EnterYourEmail','$CreateaPassword')";
