...
if(isset($_POST["submit"])){
  ...
  $result = mysqli_query($conn, "SELECT * FROM tbl_users WHERE username = '$usernameemail' AND user_password = '$password'");
  ...
  if(mysqli_num_rows($result) > 0){
    if($password == $row['user_password']){
      ...
      $email = $row["email"];
      $fullname = $row["name"];
      $query = "INSERT INTO login_user VALUES('','$email','$fullname')";
